...
function defaultFetch(_player, input, init, proxy) {
  ...
  if (proxy) {
    ...
    return import_youtubei2.Platform.shim.fetch(input, requestInit);
  } else {
    return import_youtubei2.Platform.shim.fetch(input, requestInit);
  }
}
...
var import_youtube_dl_exec2 = __toESM(require("youtube-dl-exec"));
var import_promises = require("fs/promises");
var import_promises2 = require("stream/promises");
var import_node_fs = require("fs");
var { YOUTUBE_DL_PATH, YOUTUBE_DL_HOST, YOUTUBE_DL_DIR, YOUTUBE_DL_FILE } = import_youtube_dl_exec2.default.constants;
...
async function updateDl() {
  const [binary] = await Promise.all([
    getDlBinary(),
    (0, import_promises.mkdir)(YOUTUBE_DL_DIR, { recursive: true })
  ]);
  await (0, import_promises2.pipeline)(binary, (0, import_node_fs.createWriteStream)(YOUTUBE_DL_PATH));
  await (0, import_promises.chmod)(YOUTUBE_DL_PATH, 493);
}
async function generateStreamWithYoutubeDL(track, youtubei) {
  ...
  const process2 = import_youtube_dl_exec.default.exec(videoUrl, {
    jsRuntimes: "node",
    format: videoFormat,
    output: "-",
...
var YoutubeiExtractor = class _YoutubeiExtractor extends import_discord_player.BaseExtractor {
  ...
  async #createWithToken() {
    ...
    this.innerTube = await import_youtubei3.default.create({
      ...
      cache: new import_youtubei3.UniversalCache(false),
  ...
  async activate() {
    ...
    this.innerTube = await import_youtubei3.default.create({
      ...INNERTUBE_OPTIONS,
      fetch: (input, init) => defaultFetch(this.context.player, input, init, this.options.proxy),
      cache: this.options.generateWithPoToken ? new import_youtubei3.UniversalCache(false) : void 0,
  ...
  async handle(query, context) {
    ...
    switch (context.type) {
      case import_discord_player.QueryType.YOUTUBE_PLAYLIST: {
        ...
        if (videoId && plId) {
          const endpoint = new import_youtubei3.YTNodes.NavigationEndpoint({
            continuationCommand: {
              videoId,
              playlistId: plId
            }
        ...
        while (playlist.has_continuation) {
          ...
          plTracks.push(
            ...playlist.videos.filterType(import_youtubei3.YTNodes.PlaylistVideo).map((v) => {
              const duration = import_discord_player.Util.buildTimeCode(
                import_discord_player.Util.parseMS(v.duration.seconds * 1e3)
              );
              const raw = {
      ...
      default: {
        ...
        const videos = search.videos.filterType(import_youtubei3.YTNodes.Video);
        return {
          playlist: null,
